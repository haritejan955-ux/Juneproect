...
and `backend/app/memory/checkpointer.py` — those files already exist and pass 27 tests (see the
previous milestone). **No agent node logic is being added or changed in this pass** — this is
the design record for what's already wired, written in the form requested: state, nodes, edges,
routing, retry loop, memory, checkpointing, audit log, and shared state, each explained on its
own, with diagrams.
...
- **Validation already happens at the right layer.** Each node that calls an LLM parses the
  ...
  *before* it ever touches `GraphState`. Re-validating a second time at the state layer would be
  ...
  validation boundary.
- **`total=False` encodes the graph's own shape.** A field like `coverage_map` genuinely doesn't
...
| Field(s) | Owner |
|---|---|
| `claim_id`, `claimant_id`, `query`, `raw_documents` | input (set once, before the graph runs) |
| `document_chunks`, `document_metadata`, `pii_flags` | Document Preprocessor |
| `intent`, `intent_confidence`, `extracted_entities` | Intent Analyzer |
| `retrieved_chunks`, `low_confidence_retrieval` | RAG Retriever |
| `security_flags`, `injection_detected`, `redacted_chunks` | Security Checker |
| `coverage_map`, `coverage_citations` | Coverage Validator |
| `fraud_signals`, `attorney_flag` | Fraud Detector |
| `draft_decision`, `justification`, `disclaimer` | Answer Synthesizer |
| `critic_score`, `critique`, `retry_count` *(read)*, `low_confidence` | Self-Critic |
| `retry_count` *(write)* | `prepare_retry` (control-flow node — see [Retry Loop](#5-retry-loop)) |
| `final_decision` | Final Output |
| `audit_log` | every node (accumulating — see [Audit Log](#8-audit-log)) |

### The one field with a reducer: `audit_log`
...
the previous value — safe here only because each field has exactly one writer). `audit_log` is
different: it must accumulate across *every* node, so it's declared with LangGraph's
`Annotated[type, reducer]` pattern. When node N returns `{"audit_log": [entry]}`, LangGraph
doesn't overwrite the list — it calls `operator.add(existing_list, [entry])`, i.e. concatenates.
This is the only reducer in the schema; everything else relies on the ownership rule above,
which is enough because no two nodes ever write the same field.
...
**LLM-free, deterministic.** Parses uploaded PDFs (PyMuPDF), chunks by clause/section boundary —
never a fixed token window, since that's the spec's named failure mode — preserving CPT/ICD-10
codes and policy section numbers verbatim via regex extraction. Flags SSN/DOB/EIN candidates via
regex + field-label proximity *before any LLM call exists anywhere in the run*. Writes
`document_chunks`, `document_metadata`, `pii_flags`.
...
from `document_chunks`, the persistent `policy_corpus` index, and the persistent
`historical_decisions` index — and merges results with source metadata attached to every chunk.
Results below the configurable similarity threshold are kept and flagged
(`low_confidence: true`), not silently dropped. Writes `retrieved_chunks`,
`low_confidence_retrieval`.

### 4 — Security Checker
**Hybrid: regex/heuristic layer + LLM classifier layer.** Runs both detection layers against the
raw document text; either flagging above its confidence threshold sets
`injection_detected = true`. Also produces `redacted_chunks` (PII masked out of
`retrieved_chunks`) for every downstream node to consume instead of the raw chunks. This node
only *sets a flag* — the actual hard stop is enforced by the conditional edge described in
[Conditional Routing](#4-conditional-routing). Writes `security_flags`, `injection_detected`,
`redacted_chunks`.
...
`coverage_map`, `coverage_citations`.

### 6 — Fraud Detector
**LLM, structured output.** Scores the claim against all four required signal types (duplicate
billing, upcoding, date conflict, unbundling) using `coverage_map` and `document_chunks`. Each
reported signal carries `severity` and an `evidence` string. `attorney_flag` is computed here by
plain Python (`any(signal.severity == "high" ...)`), not requested from the model. Writes
`fraud_signals`, `attorney_flag`.

### 7 — Answer Synthesizer
**LLM, structured output.** Produces `draft_decision` (`approved | partial_approved | denied`)
and `justification`, grounded only in `redacted_chunks`. On a retry, its prompt includes the
prior `critique` as a mandatory section (not optional context) — see [Retry Loop](#5-retry-loop).
The legal disclaimer is appended by plain code immediately after the LLM call returns, never
requested from the model. Writes `draft_decision`, `justification`, `disclaimer`.
...
hallucination risk (30%), producing `critic_score` and, if the score is low, a specific
`critique`. It also computes `low_confidence` itself — `True` only when the score is below
...
`critic_score`, `critique`, `low_confidence`.

### 9 — Final Output
**LLM-free, deterministic assembly.** Reads the accumulated state and assembles the single
`final_decision` object the API and frontend consume — no new reasoning happens here, it's a
pure read of what nodes 1–8 already produced. Writes `final_decision`.

### `blocked` (control-flow)
Terminal node reached only when Security Checker confirms an injection. Assembles a
`final_decision` with `status: "blocked"` and writes the one audit entry that records *why* the
pipeline halted. No downstream domain agent (5 through 9) ever executes for a blocked claim —
this is enforced structurally by the graph topology, not by a flag agents are expected to check.
...
def route_after_security(state: GraphState) -> Literal["coverage_validator", "blocked"]:
    return "blocked" if state.get("injection_detected") else "coverage_validator"

def route_after_critic(
    state: GraphState, score_threshold: float, max_retries: int
) -> Literal["prepare_retry", "final_output"]:
    if state.get("critic_score", 0.0) >= score_threshold:
        return "final_output"
...
flowchart TD
    ...
    D1 -->|injection_detected = True| BL[blocked]
    D1 -->|injection_detected = False| CV[coverage_validator]

    SCR[self_critic] --> D2{route_after_critic}
    D2 -->|score >= threshold| FO[final_output]
    D2 -->|retries exhausted| FO
    D2 -->|score low, retries remain| PR[prepare_retry]
```
...
1. **Critique injection is structural, not optional.** `answer_synthesizer_prompt.py`'s template
   unconditionally adds an "address the following issues" section whenever `state["critique"]`
...
`.ainvoke(...)` call against the compiled graph is checkpointed automatically; no node is aware
it's happening.
...
the same thread* — the checkpointer already holds `coverage_map`, `fraud_signals`,
`retrieved_chunks`, and the new `critique`, so nothing is re-fetched. This is the literal
mechanism behind *"no agent re-fetches data that a previous agent already produced."*
...
sequenceDiagram
    ...
    alt score too low, retries remain
...
*long-term* store — see `docs/memory-architecture.md`). The checkpointer is never queried by
anything outside this one service method; the durable, queryable record other parts of the app
read from is SQLite, not the checkpointer.
...
uses the one shared helper instead of constructing entries ad hoc.
...
- **No overwriting:** the field-ownership table in [State](#1-state) is exhaustive — each field
  ...
  dicts share a key (except `audit_log`, which has the reducer specifically because it's the one
  intentional exception), a shallow update is equivalent to a safe merge — there's no scenario
  where node N+1's return value clobbers a field node N wrote.

```mermaid
flowchart TB
    subgraph State["Single GraphState object, threaded through every node"]
        direction LR
        F1[claim_id / query / raw_documents]
        F2[document_chunks / pii_flags]
        F3[intent / entities]
        F4[retrieved_chunks]
        F5[security_flags / redacted_chunks]
        F6[coverage_map]
        F7[fraud_signals / attorney_flag]
        F8[draft_decision / justification]
        F9[critic_score / critique / retry_count]
        F10[final_decision]
